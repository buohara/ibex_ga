...
This branch includes an experimental **Conformal Geometric Algebra (CGA) ALU coprocessor** integrated with the Ibex RISC-V core with extended instruction set. The GA coprocessor accelerates geometric computations commonly performed with matrix/vector arithmetic with single-cycle custom ops that provide useful generalizations of complex, quaternion, and octonion arithmetic. Applications include 6-DOF robotics, computer graphics transformations, and multiphysics simulation acceleration.
